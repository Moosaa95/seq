...
class PropertyViewSet(viewsets.ModelViewSet):
    """
    ViewSet for Parent Property (Building/Complex/Site) management.
    """
...
class ApartmentViewSet(viewsets.ModelViewSet):
    """
    ViewSet for Apartment Unit management (Bookable Units).
    
    Filters:
    - property: filter by parent building id
    - status: 'rent' or 'sale'
    - type: apartment type
    ...
    filterset_fields = ["parent_property", "status", "featured"]
    search_fields = ["title", "parent_property__location__name", "description", "type", "entity"]
    ...
    def get_queryset(self):
        ...
        # Filter by property (parent building)
        property_id = self.request.query_params.get("property", None)
        if property_id:
            queryset = queryset.filter(parent_property_id=property_id)
    ...
    def booked_dates(self, request, pk=None):
        """
        Get list of booked/blocked dates for the apartment unit.
        """
        ...
        ranges = []
        for b in bookings:
            ranges.append({"start": b["check_in"], "end": b["check_out"]})
            
        for b in blocked:
            ranges.append({"start": b["start_date"], "end": b["end_date"]})
    ...
    def availability(self, request, pk=None):
        """
        Check apartment unit availability for given dates
        """
        apartment_obj = self.get_object()
        ...
        try:
            ...
            is_available = ICalService.check_availability_with_blocked_dates(
                apartment_obj, check_in_date, check_out_date
            ) and apartment_obj.is_available
...
class ApartmentInquiryViewSet(viewsets.ModelViewSet):
    """
    ViewSet for Apartment-specific Inquiry management.
    """
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Failed to send apartment inquiry notification: {str(e)}")
...
@api_view(["GET"])
@permission_classes([AllowAny])
def export_apartment_ical(request, apartment_id):
    """
    Export apartment bookings as iCal feed.
    """
    try:
    ...
    try:
        ical_data = ICalService.export_property_calendar(apartment_obj)
        response = HttpResponse(ical_data, content_type="text/calendar; charset=utf-8")
        response["Content-Disposition"] = f'attachment; filename="{apartment_obj.title.replace(" ", "_")}_calendar.ics"'
        return response
    except Exception as e:
...
class PropertyInventoryViewSet(viewsets.ModelViewSet):
    """
    ViewSet for PropertyInventory (Building Supply stock).
    """
...
class ApartmentInventoryViewSet(viewsets.ModelViewSet):
    """
    ViewSet for ApartmentInventory (Unit Supply stock).
    """

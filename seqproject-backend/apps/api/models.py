...
class Property(ModelMixins):
    """Parent building/complex/site (e.g. 'San Gwari District')"""
    ...
    address = models.TextField(blank=True, null=True, help_text='Full street address of the property')
    latitude = models.DecimalField(
        max_digits=9, decimal_places=6, blank=True, null=True,
        help_text='GPS latitude (e.g. 9.072264)',
    )
    longitude = models.DecimalField(
        max_digits=9, decimal_places=6, blank=True, null=True,
        help_text='GPS longitude (e.g. 7.491302)',
    )
...
class PropertyImage(ModelMixins):
    """Images for a property (parent building/site)"""
    ...
    category = models.CharField(
        ...
        blank=True,
        null=True,
        help_text="e.g., Exterior, Interior, Amenities, Construction",
    )
...
class Apartment(ModelMixins):
    """Individual bookable unit (Apartment/Room) inside a Property"""

    # Basic Information
    parent_property = models.ForeignKey(
        Property, on_delete=models.CASCADE, related_name="apartments", null=True, blank=True
    )
    ...
    agent = models.ForeignKey(
        Agent, on_delete=models.SET_NULL, null=True, related_name="apartments"
    )
...
class ApartmentImage(ModelMixins):
    """Apartment images with optional categorization"""
    ...
    def __str__(self):
        return f"{self.apartment.title} - Image {self.order}"
...
class ApartmentInquiry(ModelMixins):
    """Apartment-specific inquiries"""

    apartment = models.ForeignKey(
        Apartment, on_delete=models.CASCADE, related_name="inquiries", null=True, blank=True
    )
    name = models.CharField(max_length=255)
    email = models.EmailField()
    phone = models.CharField(max_length=50)
...
class ExternalCalendar(ModelMixins):
    """External calendar feeds for apartment synchronization"""

    apartment = models.ForeignKey(
        Apartment, on_delete=models.CASCADE, related_name="external_calendars", null=True, blank=True
    )
    ...
    def __str__(self):
        return f"{self.apartment.title} - {self.get_source_display()}"
...
class BlockedDate(ModelMixins):
    ...
    apartment = models.ForeignKey(
        Apartment, on_delete=models.CASCADE, related_name="blocked_dates", null=True, blank=True
    )
    ...
    def __str__(self):
        return f"{self.apartment.title} - {self.start_date} to {self.end_date}"
...
class PropertyInventory(ModelMixins):
    """Inventory stock assigned to a parent property/building/site (e.g., cement)"""
    ...
    def __str__(self):
        return f"{self.property.name} - {self.item.name}: {self.quantity}"


class ApartmentInventory(ModelMixins):
    """Inventory assigned to a specific bookable apartment unit (e.g., bedsheets)"""

    apartment = models.ForeignKey(
        Apartment, on_delete=models.CASCADE, related_name="inventory", null=True, blank=True
    )
    item = models.ForeignKey(
        InventoryItem, on_delete=models.CASCADE, related_name="apartment_assignments"
    )
    quantity = models.IntegerField(default=0, validators=[MinValueValidator(0)])
    ...
    def __str__(self):
        return f"{self.apartment.title} - {self.item.name}: {self.quantity}"
